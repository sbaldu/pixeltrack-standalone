...
def pixeltrack(x):
    ...
    # print(x)
    try:
        subprocess.check_call(['./serial', '--maxEvents', '24', '--numberOfThreads', '0','--objective', '--paramsFromFile'])
    ...
    return [objective['fake_rate'], objective['efficiency']]

patatune.Randomizer.rng = np.random.default_rng(42)

patatune.FileManager.working_dir = "optimization/"
...
objective = patatune.ElementWiseObjective([pixeltrack], 2, objective_names=['fake_rate', 'efficiency'], directions=['minimize', 'maximize'])
