...
class TrackMLHistoValidator : public edm::EDProducer {
public:
  explicit TrackMLHistoValidator(edm::ProductRegistry& reg);
...
std::map<std::string, SimpleAtomicHisto> TrackMLHistoValidator::histos = {
...
TrackMLHistoValidator::TrackMLHistoValidator(edm::ProductRegistry& reg)
    : hitToken_(reg.consumes<TrackingRecHit2DCPU>()),
      trackToken_(reg.consumes<PixelTrackHeterogeneous>()) {}

void TrackMLHistoValidator::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
void TrackMLHistoValidator::endJob() {
...
DEFINE_FWK_MODULE(TrackMLHistoValidator);
